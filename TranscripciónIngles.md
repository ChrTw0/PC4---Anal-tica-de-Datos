Entendido. Aquí tienes la transcripción textual del contenido del PDF, página por página, respetando el idioma original (inglés) y el formato del documento, sin interpretaciones ni resúmenes.
...
[Diagram containing: tf.keras, tf.estimator (High-level Deep Learning APIs); tf.nn, tf.losses, tf.metrics, tf.optimizers, tf.train, tf.initializers (Low-level Deep Learning APIs); tf.GradientTape, tf.gradients() (Autodiff); tf.data, tf.feature_column, tf.audio, tf.image, tf.io, tf.queue (I/O and preprocessing); tf.distribute, tf.saved_model, tf.autograph, tf.graph_util, tf.lite, tf.quantization, tf.tpu, tf.xla (Deployment and optimization); tf.lookup, tf.nest, tf.ragged, tf.sets, tf.sparse, tf.strings (Special data structures); tf.math, tf.linalg, tf.signal, tf.random, tf.bitwise (Mathematics); tf.summary (Visualization); tf.compat, tf.config (Miscellaneous)]
...
[Diagram showing flow from "Your Python code" -> Keras / Data API -> Low-level Python API (ops) -> C++ / Go APIs -> Local/distributed execution engine -> CPU kernels / GPU kernels / TPU kernels]
...
1
 ...
 regularizer (equivalent to keras.regularizers.l1(0.01)), and a custom constraint that ensures weights are all positive (equivalent to keras.constraints.nonneg() or tf.nn.relu()):
...
1
 ...
 regularization that saves its factor hyperparameter (this time we do not need to call the parent constructor or the get_config() method, as they are not defined by the parent class):
...
[Diagram showing Input -> Dense -> ResidualBlock (x3) -> ResidualBlock -> Dense -> Output. A ResidualBlock contains two Dense layers and an addition operation (+)]
...
This is because computing the gradients of this function using autodiff leads to some numerical difficulties: due to floating-point precision errors, autodiff ends up computing infinity divided by infinity (which returns NaN). Fortunately, we can analytically find that the derivative of the softplus function is just 
1
/
(
1
...
AutoGraph: Takes Python code def sum_squares(n):... -> generates def tf__sum_squares(n):... using tf.while_loop and loop_body.

Tracing: Calls tf__sum_squares with symbolic Tensor (name="n:0", shape=(), dtype=int32).
Result: Graph generation with nodes like tf.while_loop, tf.range, n.]*
...
𝛼
α
 should be initialized with 1s, and 
𝛽
β
 with 0s.
b. The call() method should compute the mean 
𝜇
μ
 and standard deviation 
𝜎
σ
 of each instance’s features. For this, you can use tf.nn.moments(inputs, axes=-1, keepdims=True), which returns the mean 
𝜇
μ
 and the variance 
𝜎
...
σ
2
 of all instances (compute the square root of the variance to get the standard deviation). Then the function should compute and return 
𝛼
⊗
(
𝑋
...
⊗
⊗
 represents itemwise multiplication (*) and 
𝜖
ϵ
